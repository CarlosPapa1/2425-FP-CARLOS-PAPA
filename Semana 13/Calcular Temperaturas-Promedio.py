...
def calcular_promedio(temperaturas):
    ...
    Args:
        temperaturas (list): Matriz 3D de temperaturas con estructura (Ciudades, Semanas, Días)
    """
    for ciudad_idx, ciudad in enumerate(temperaturas):
        for semana_idx, semana in enumerate(ciudad):
            suma_temperatura = sum(dia["temp"] for dia in semana)
            promedio = suma_temperatura / len(semana)
            print(f'Promedio en {ciudades[ciudad_idx]}, semana {semana_idx + 1}: {promedio:.2f}°C')
